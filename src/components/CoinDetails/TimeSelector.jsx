...
const TimeSelector = ({ time, timePeriod, setTimePeriod }) => {
  return (
    <Listbox value={timePeriod} onChange={setTimePeriod}>
      <div className="relative mt-1">
        <Listbox.Button className="relative w-full py-1.5 pl-4 pr-10 text-left bg-white rounded-lg shadow-md cursor-default focus:outline-none focus-visible:ring-2 focus-visible:ring-opacity-75 focus-visible:ring-white focus-visible:ring-offset-orange-300 focus-visible:ring-offset-2 focus-visible:border-indigo-500 sm:text-sm">
          <span className="block truncate">{timePeriod}</span>
              ...
              >
                <span
                  className={`truncate flex gap-2 ${
                    timePeriod === unit ? "font-medium" : "font-normal"
                  }`}
                >
                  {unit}
                </span>
